...
// Initialise folder as to allow git fetch to work
if (!fs.existsSync(localDirectory + "\.git")) {
  console.log(localDirectory + "\.git")
  console.log("working")
  exec('git init', { cwd: localDirectory, encoding: 'utf-8' })
  exec('git add --all', { cwd: localDirectory, encoding: 'utf-8' })
  exec("git commit -m 'Add files'", { cwd: localDirectory, encoding: 'utf-8' })
  exec('git remote add origin https://github.com/Rumodeus/osiris', { cwd: localDirectory, encoding: 'utf-8' })
  exec('git fetch origin', { cwd: localDirectory, encoding: 'utf-8' })
  exec('git diff origin/master', { cwd: localDirectory, encoding: 'utf-8' })
}
